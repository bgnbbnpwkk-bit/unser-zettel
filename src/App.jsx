...
export default function App() {
 ...
 const finishShopping = async () =>
   Promise.all(items.filter(i => i.checked).map(i =>
     updateDoc(doc(db, 'items', i.id), { checked: false })
   ))
       ...
       <div style={S.list}>
         ...
         {checkedCount > 0 && activeFilter !== 'checked' && (
           <button style={S.finishBtn} onClick={finishShopping}>
             🛒 Einkauf beenden · {checkedCount} erledigt
           </button>
         )}
       </div>
           ...
           <div style={S.panelInner}>
             ...
             <div style={{ display:'flex', flexDirection:'column', gap:16, maxHeight:360, overflowY:'auto' }}>
               ...
               <div>
                 ...
                 {['Favoriten als Filter','App-Name aktualisiert','Bugfixes'].map(c => (
                   <div key={c} style={{ color:'rgba(255,255,255,0.7)', fontSize:12, paddingBottom:3 }}>+ {c}</div>
                 ))}
...
function ItemRow({ item, onToggle, onRemove, catColor, addedByColor, isFav, onToggleFav, itemName }) {
 return (
